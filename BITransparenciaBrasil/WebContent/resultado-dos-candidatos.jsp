...
<%@ page import="java.util.*, databeans.*" %>
<%@ taglib prefix="s" uri="/struts-tags"%>
<%
  String path = request.getContextPath();
	String estado = request.getParameter("estado");
	String cargo = request.getParameter("cargo");
	Collection<String[]> dadosCandidatos = null;
	if (estado != null && estado.trim().length() > 0) dadosCandidatos = QueryBuilder.getCandidatos(estado, cargo);
	
	String nomeEstado = QueryBuilder.nomeEstado(estado);
%> 
      ...
      <section class="features" id="tecnologia">
        <div class="container">
          <div class="row">
                  ...
                  <strong>Escolha o candidato</strong><br><br>
                  </h1>
                  <h3 class="tile-title"><%=nomeEstado %></h3>
                  </div>
           		<div class="col-md-12" align="center" >
              		
                    <div class="row demo-tiles">
                    <%
                    	Iterator<String[]> i = dadosCandidatos.iterator();
                    	int b = 0; 
                    	while (i.hasNext()) {
                    		String [] dados = i.next();
                    
                    		if (b++%4==0) { 
                           		%></div><br><br><div class="row demo-tiles" ><%
                           	}
                    	%>
	                        <div class="col-xs-3"  style="height:400px">
	                          <div class="tile" style="height:350px">
	                            <img src="<%=dados[0] %>" alt="Compas" class="tile-image big-illustration" style="border-style:solid; border-color:cecece;border-width:1px;">
	                            <h3 class="tile-title"><b><%=dados[4] %> / <%=dados[2] %></b></h3>
	                            <h3 class="tile-title"><%=dados[3] %> <%=(estado != null && !estado.equals("0") ? "/ " + estado.toUpperCase() : "") %></h3>
